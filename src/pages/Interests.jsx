...
export default function Interests() {
  ...
  const filtered = interests.filter((i) => {
    ...
    return (
      ...
      (i.puja?.pujaName || i.puja?.pujaType)?.toLowerCase().includes(q) ||
              ...
              <tbody>
                {paginated.map((i) => (
                  <tr key={i._id} className="border-t border-gray-50 transition-colors hover:bg-orange-50">
                    ...
                    <td className="px-8 py-4 whitespace-nowrap">
                      <p className="font-bold" style={{ color: THEME }}>{(i.puja?.pujaName || i.puja?.pujaType)?.trim() || "No Puja"}</p>
                    </td>
                    ...
                    <td className="px-8 py-4">
                      <button onClick={() => setEditData({
                          ...
                          name: i.name,
                          mobile: i.mobile,
                          puja: (i.puja?.pujaName || i.puja?.pujaType)?.trim() || "No Puja"
                        })}
                        title="Update & Notes"
